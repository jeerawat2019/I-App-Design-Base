...
namespace MCore.Comp.VisionSystem
{
    ...
    public class CognexCamera7 : CameraBase
    {
        ...
        public CognexCamera7(string name)
            : base(name)
        {
        }
        ...
        public override string[] GetCameraIDs()
        {
            ...
            if (Parent is Cognex7)
            {
                lock (_lockAcqFifo)
                {
                    CogFrameGrabbers grabbers = (Parent as Cognex7).FramegrabberCams;
                    if (grabbers != null)
                    {
        ...
        {
            return CognexJob7.CogImageToBitmap(oImage);
        }
                    ...
                    try
                    {
                        ...
                        if (numReady > 0)
                        {
                            ...
                            if (cogImage is CogImage8Grey)
                            {
                                ...
                                //CogImage8Grey cogImage8Grey = _mOneImageTool.OutputImage;
                                CogImage8Grey cogImage8Grey = cogImage as CogImage8Grey;
                                if (BayerType != -1)
                                {
                                    _mConGrey.RunParams.RunMode = CogImageConvertRunModeConstants.RGBFromBayer;
                                    _mConGrey.RunParams.BayerType = (CogImageConvertBayerTypeConstants)BayerType;
                                }
                                //_mConGrey.RunParams.ColorFromBayerRedScale = 1;
                                //_mConGrey.RunParams.ColorFromBayerGreenScale = 1;
                                //_mConGrey.RunParams.ColorFromBayerBlueScale = 1;
                                _mConGrey.InputImage = cogImage8Grey;
                                _mConGrey.Run();
                                _mConGrey.InputImage = null;
                                (cogImage as IDisposable).Dispose();
                                return _mConGrey.OutputImage as CogImage24PlanarColor;
                            }
