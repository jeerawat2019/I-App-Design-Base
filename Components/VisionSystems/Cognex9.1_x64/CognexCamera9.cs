...
namespace MCore.Comp.VisionSystem
{
    ...
    public class CognexCamera9 : CameraBase
    {
        ...
        public CognexCamera9(string name)
            : base(name)
        {
        }
        ...
        public override string[] GetCameraIDs()
        {
            ...
            if (Parent is Cognex9)
            {
                lock (_lockAcqFifo)
                {
                    CogFrameGrabbers grabbers = (Parent as Cognex9).FramegrabberCams;
                    if (grabbers != null)
                    {
        ...
        {
            return CognexJob9.CogImageToBitmap(oImage);
        }
